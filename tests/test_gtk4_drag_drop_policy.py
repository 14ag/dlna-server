...
def test_should_allow_source_drop_matches_windows_policy(dlna_server_binary):
    ...
    allowed = subprocess.run(
        [dlna_server_binary, "--print-should-allow-source-drop", "0"],
        capture_output=True, text=True, timeout=5,
    )
    disallowed = subprocess.run(
        [dlna_server_binary, "--print-should-allow-source-drop", "1"],
        capture_output=True, text=True, timeout=5,
    )
