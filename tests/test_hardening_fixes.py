...
class HardeningFixSourceTests(unittest.TestCase):
    ...
    def test_remote_fetch_uses_direct_process_without_shell_pipe(self):
        source = self.read("src/network_sources.cpp")

        for forbidden in ("popen", "_wpopen", "_popen", "system("):
            self.assertNotIn(forbidden, source)
        self.assertIn("CreateProcessW", source)
        self.assertIn("execvp", source)
        self.assertIn("RunCurlWithReader", source)
        self.assertIn("Always drain to avoid blocking curl", source)
        self.assertIn("TerminateProcess", source)
        self.assertIn("kill(child, SIGTERM)", source)
    ...
    def test_ports_backups_ci_and_docs_are_hardened(self):
        ...
        py_gui = self.read("src/posix_gui.py")
        ...
        self.assertIn("_parse_port", py_gui)
